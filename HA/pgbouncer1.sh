...
run_pgbench_test() {
    ...
    echo ""
    log_info "High concurrency test - Direct connection (expected to fail)..."
    ${PGBENCH_BIN} -c 200 -t 10 -S -U "${DB_USER}" -h ${BACKEND_HOSTNAME} -p ${BACKEND_PORT} postgres 2>&1 | head -5 || true
    log_warning "Direct connection fails with too many clients (expected)"
    
    echo ""
    log_info "High concurrency test - Via PgBouncer (200 clients)..."
    ${PGBENCH_BIN} -c 200 -t 10 -S -U "${DB_USER}" -h 127.0.0.1 -p ${PGBOUNCER_PORT} postgres 2>/dev/null || {
        log_warning "High concurrency test failed"
    }
    log_success "PgBouncer handles 200 concurrent clients successfully!"
...
print_summary() {
    ...
    echo ""
    echo "High Concurrency Test (demonstrates PgBouncer benefit):"
    echo "  - Direct (will fail):     pgbench -c 200 -t 10 -S -U ${DB_USER} -h ${BACKEND_HOSTNAME} -p ${BACKEND_PORT} postgres"
    echo "  - Via PgBouncer (works):  pgbench -c 200 -t 10 -S -U ${DB_USER} -h localhost -p ${PGBOUNCER_PORT} postgres"
    echo ""
    echo "==============================================================================="
}
