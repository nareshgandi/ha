...
show_menu() {
    echo "==============================================================================="
    echo "           PATRONI FAILOVER TESTING - INTERACTIVE MENU"
    echo "==============================================================================="
    echo ""
    show_cluster_status
    echo ""
    ...
    echo "  12. Cascade Failure (Multiple nodes)"
    echo "  13. Watchdog Test"
    echo "  14. Recovery Test (Rejoin node)"
    echo ""
    echo "STATUS & HEALTH:"
    echo "  s.  Show Cluster Status"
    echo "  v.  Test VIP Connection"
    echo "  h.  Full Health Check"
    echo ""
    echo "RECOVERY - Start Services:"
    echo "  r.   Restore All (start all services on all nodes)"
    echo "  r1.  Start all services on lab01"
    echo "  r2.  Start all services on lab02"
    echo "  r3.  Start all services on lab03"
    echo "  rp.  Start Patroni only (all nodes)"
    echo "  re.  Start etcd only (all nodes)"
    echo "  rh.  Start HAProxy only (all nodes)"
    echo "  rk.  Start Keepalived only (all nodes)"
    echo ""
    echo "RECOVERY - Stop Services:"
    echo "  sp.  Stop Patroni (all nodes)"
    echo "  se.  Stop etcd (all nodes)"
    echo "  sh.  Stop HAProxy (all nodes)"
    echo "  sk.  Stop Keepalived (all nodes)"
    echo ""
    echo "RECOVERY - Fix Issues:"
    echo "  fn.  Fix Network (clear iptables all nodes)"
    echo "  ri.  Reinitialize a failed node"
    echo ""
    ...
    case $choice in
        ...
        v|V) test_vip_connection ;;
        h|H) full_health_check ;;
        
        # Recovery - Start All
        r|R) 
            log_info "Restoring all nodes..."
            for node in lab01 lab02 lab03; do
                restore_node "$node"
            done
            sleep 20
            ...
            ;;
        r1)
            log_info "Starting all services on lab01..."
            start_all_on_node "lab01"
            sleep 10
            show_cluster_status
            ;;
        r2)
            log_info "Starting all services on lab02..."
            start_all_on_node "lab02"
            sleep 10
            show_cluster_status
            ;;
        r3)
            log_info "Starting all services on lab03..."
            start_all_on_node "lab03"
            sleep 10
            show_cluster_status
            ;;
            
        # Recovery - Start Individual Services
        rp|RP)
        ...
        se|SE)
            ...
            for node in lab01 lab02 lab03; do
                log_step "Stopping etcd on ${node}..."
        ...
        sh|SH)
            ...
            for node in lab01 lab02 lab03; do
                log_step "Stopping HAProxy on ${node}..."
        ...
        sk|SK)
            ...
            for node in lab01 lab02 lab03; do
                log_step "Stopping Keepalived on ${node}..."
        ...
        # Recovery - Fix Issues
        fn|FN)
            log_info "Fixing network (clearing iptables) on all nodes..."
            for node in lab01 lab02 lab03; do
                log_step "Clearing iptables on ${node}..."
                run_on_node "$node" "iptables -F 2>/dev/null || true"
            done
            log_success "Network fixed on all nodes"
            ;;
        ri|RI)
            echo ""
            read -p "Enter node to reinitialize (lab01/lab02/lab03): " reinit_node_name
            reinit_failed_node "$reinit_node_name"
            ;;
