...
if [[ $# -lt 4 ]]; then
    ...
    echo ""
    echo "Options:"
    echo "  -i, --interface IFACE   Network interface (default: ens33)"
...
install_patroni() {
    ...
    run_on_all_nodes "
        ...
        # Fix patronictl if it's a broken symlink
        if [ -L /usr/bin/patronictl ] && [ ! -e /usr/bin/patronictl ]; then
            rm -f /usr/bin/patronictl
            cat > /usr/bin/patronictl << 'CTLEOF'
#!/usr/bin/python3.12
import sys
try:
